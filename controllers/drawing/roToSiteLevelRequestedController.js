...
exports.createRequest = catchAsync(async (req, res, next) => {
  ...
  try {
    // SAFE register fetch
    const registerData1 = await ArchitectureToRoRegister.findById(drawingId);
    ...
    req.body.designDrawingConsultant =
      registerData1.designDrawingConsultant;
    req.body.folderId = registerData1.folderId;

    // prevent duplicate revision
    ...
    if (drawingNo) {
      ...
      req.body.roRfiNo = number ? number.padStart(3, "0") : "000";
    }
...
const updatedRegister = await ArchitectureToRoRegister.findOneAndUpdate(
  ...
  },
  {
    ...
    // SAFE architect lookup
    let roToSiteLevelRequest = null;

    if (latestRevision) {
      roToSiteLevelRequest = await Architecture.findOne({
        drawingId,
    ...
    if (siteHeadIds?.length > 0) {
      for (let user of siteHeadIds) {
        const sitePermission = user?.permittedSites?.find(
          (site) => site.siteId.toString() === req.body.siteId
        );

        const access =
          sitePermission?.enableModules?.drawingDetails?.roDetails
            ?.rfiRaisedAccess || false;

        if (access) {
          const notificationMessage =
            `A RFI has been raised for drawing number ${drawingNo} with architect revision ${revision}.`;
    ...
    // update forwarded request safely
    if (siteHeadRfiId) {
...
const updatedRegister = await ArchitectureToRoRegister.findOneAndUpdate(
  { drawingNo, siteId },
  {
    $set: {
      ...
      // Architect revisions
      "acceptedArchitectRevisions.$[arch].siteHeadRfiStatus": "Rejected",
      "acceptedArchitectRevisions.$[arch].rfiRejectStatus": "Rejected",

      // RO siteHeadRfiStatus
      "acceptedRORevisions.$[ro].siteHeadRfiStatus": "Rejected",

      // SiteHead revisions
      "acceptedSiteHeadRevisions.$[site].siteHeadRfiStatus": "Rejected",
  ...
  },
  {
    ...
    runValidators: true,
    arrayFilters: [
      { "elem.revision": revision },  // for acceptedRORevisions rfiStatus + rfiRejectStatus
      { "ro.revision": revision },    // for acceptedRORevisions siteHeadRfiStatus
      { "arch.revision": revision },  // for acceptedArchitectRevisions
      { "site.revision": revision },  // for acceptedSiteHeadRevisions
    ],
  }
);
...
await ArchitectureToRoRegister.findOneAndUpdate(
  { _id:drawingId, siteId },
  {
    $set: {
      // Architect
      "acceptedArchitectRevisions.$[arch].siteHeadRfiStatus": requestStatus,

      // RO
      "acceptedRORevisions.$[ro].siteHeadRfiStatus": requestStatus,
      "acceptedRORevisions.$[ro].rfiStatus": "Raised",

      // SiteHead
      "acceptedSiteHeadRevisions.$[site].siteHeadRfiStatus": requestStatus,
    },
  },
  {
    new: true,
...
exports.closeRequest = catchAsync(async (req, res, next) => {
  ...
  const updatedRegister = await ArchitectureToRoRegister.findOneAndUpdate(
      { drawingNo, siteId },
      {
        $set: {
          // Architect
          "acceptedArchitectRevisions.$[arch].siteHeadRfiStatus": "Closed",
  
          // RO
          "acceptedRORevisions.$[ro].siteHeadRfiStatus": "Closed",
          "acceptedRORevisions.$[ro].rfiStatus": "Not Raised",
  
          // SiteHead
          "acceptedSiteHeadRevisions.$[site].siteHeadRfiStatus": "Closed",
...
exports.reopenRequest = catchAsync(async (req, res, next) => {
  ...
  // );
   const updatedRegister = await ArchitectureToRoRegister.findOneAndUpdate(
    ...
    {
      $set: {
        "acceptedArchitectRevisions.$[arch].siteHeadRfiStatus": "ReOpened",
        "acceptedRORevisions.$[ro].siteHeadRfiStatus": "ReOpened",
        "acceptedSiteHeadRevisions.$[site].siteHeadRfiStatus": "ReOpened",
      },
    },
    {
      new: true,
...
exports.updateViewDates = catchAsync(async (req, res, next) => {
  const { _id } = req.body;
  const { status } = req.query; // 👈 from query
  const userId = req.user.id;
  ...
  // ✅ 1. STATUS UPDATE (always allowed)
 if (status === "Responded") {
      updateData.$set = { status: "Responded" };
    }

  // ✅ 2. VIEWED BY (only if not viewed)
  if (!alreadyViewed) {
    updateData.$push = {
      viewedBy: {
        user: userId,
