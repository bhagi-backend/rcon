...
exports.getAllSiteHeadforDrawingtab = catchAsync(async (req, res, next) => {
  ...
  if (filterType === "upload") {
      ...
      data
        ...
        .map(async (item) => {
          const enrichedSiteHeadRevisions = await Promise.all(
            item.acceptedSiteHeadRevisions.map(async (siteHeadRevision) => {
                  ...
                  if (matchedArchitectRevision) {
                    ...
                    const enrichedRevision = {
                      ...
                      currentRevisionSoftCopySubmittedDate:
                        siteHeadRevision.softCopySubmittedDate,
                    };
      ...
      data
        ...
        .map(async (item) => {
          const enrichedRevisions = await Promise.all(
            item.acceptedRORevisions.map(async (revision) => {
              if (revision.architectRef) {
                // Find matching revision in acceptedArchitectRevisions
                const matchedRevision = item.acceptedArchitectRevisions.find(
                  (archRevision) =>
                    archRevision._id.toString() ===
                    revision.architectRef.toString()
                ...
                if (matchedRevision) {
                  // Merge the matched revision with currentRevision and from fields
                  const enrichedRevision = {
                    ...matchedRevision.toObject(), // Convert to plain object if it's a Mongoose document
                    currentRevision: revision.revision,
                    currentRevisionId: revision._id,
                    from: "architect",
                    roType: revision.roType,
    ...
    return next(
      new Error("Invalid filterType. Use 'upload', 'received', or 'all'.")
    ); // Error if filterType is invalid
  }
...
exports.getAllSiteHeadForPendingTab = catchAsync(async (req, res, next) => {
  const { siteId } = req.params;
  const { filterType, folderId } = req.query;
  const userId = req.user.id;
  const userDepartment = req.user.department;
  ...
  if (filterType === "upload") {
    ...
    //                           );
    //                           return matchedRevision ? matchedRevision : revision;
    //                       }
    //                       return revision;
    //                   })
    //               );
    ...
    //                   acceptedRoRevisions: enrichedRevisions // Include the enriched revisions here
    //               };
    //           })
    //   );
    // }
...
exports.getAllSiteHeadForRegisterTab = catchAsync(async (req, res, next) => {
  const { siteId } = req.params;
        ...
        register.acceptedRORevisions.map(async (revision) => {
          if (revision.architectRef) {
            const matchedRevision = register.acceptedArchitectRevisions.find(
              (archRevision) =>
                archRevision._id.toString() === revision.architectRef.toString()
            );
            if (matchedRevision) {
              return {
                ...matchedRevision.toObject(),
          ...
          if (siteHeadRevision.roRef) {
            ...
            if (matchedSiteHead) {
              return {
                ...
                currentRevisionSoftCopySubmittedDate:
                  siteHeadRevision.softCopySubmittedDate,
              }; // Add currentRevision from siteHeadRevision
            }
            return null;
          }
              ...
              if (matchedRoRevision && matchedRoRevision.architectRef) {
                ...
                if (matchedArchitectRevision) {
                  return {
                    ...
                    currentRevisionSoftCopySubmittedDate:
                      siteRevision.softCopySubmittedDate,
                  };
                }
              }
...
exports.getAllRequestsBySiteIdForSiteHead = catchAsync(async (req, res, next) => {
